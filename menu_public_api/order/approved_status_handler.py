...
def handle(event, context):
    ...
    command = OrderApproveCommand(seller_id=seller.id, order_id=body.get('order_id'))
    handler = OrderApproveCommandHandler(order_repository=order_repository, session=session, owner=OwnerType.SELLER)
