...
def handle(event, context):
    ...
    command = OrderShipCommand(seller_id=seller.id, order_id=body.get('order_id'))
    handler = OrderShipCommandHandler(order_repository=order_repository, session=session, owner=OwnerType.SELLER)
