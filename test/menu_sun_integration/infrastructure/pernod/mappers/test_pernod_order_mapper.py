...
from menu_sun_api.domain.model.order.order import Order, OrderPayment, OrderPaymentType, OrderItem, OrderStatus, \
    OrderStatusType, OrderBillingAddress, OrderShippingAddress, OrderMetafield
...
def test_map_order_to_message():
    ...
    seller = Seller(id=2, seller_code='ABC', integration_type=IntegrationType.PERNOD,
    ...
    assert (order_dict['shipping_address']['shipping_provider'] == order.metafields[0].value)
    assert (order_dict['shipping_address']['shipping_service'] == order.metafields[1].value)
    ...
    assert (order_dict['payment_code'] == order.payments[0].payment_type.name)
