...
from test.menu_sun_integration.infrastructure.aws.sqs.mocks.order_queue_benjamin_mock import mock_aws_make_api_call

here = os.path.abspath(os.path.dirname(__file__))


class TestBenjaminProductDefaultPricingIntegrationService(IntegrationTest):
    document = "01838723032592"
    sku = "525500018"

    @pytest.fixture
    def seller(self, session):
        seller = SellerFactory.create(id=1, seller_code='ABC', integration_type=IntegrationType.BENJAMIN)
        session.commit()
        return seller
