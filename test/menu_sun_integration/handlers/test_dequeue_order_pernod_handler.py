...
class TestDequeueOrderService(IntegrationTest):
    ...
    def test_dequeue_order_from_aws_sqs_to_pernod(self, session, active_responses):
        ...
        json_file = open(
            os.path.join(
                here,
                '../infrastructure/pernod/pernod_response/send_order_response.json'))
        session.commit()
        response = json.load(json_file)
        ...
        assert result.value.metafields[0].key == "GENERAL_RULE"
        assert result.value.metafields[0].value == "ALWAYS_TRUE"
        assert result.value.seller_order_id is not None
