...
from test.menu_sun_integration.infrastructure.aws.sqs.mocks.order_status_queue_pernod_mock \
    import mock_queue_make_api_call
...
class TestDequeueUpdateOrderService(IntegrationTest):
    ...
    def test_dequeue_order_status_notification_from_aws_sqs_to_pernod(self, session, active_responses):
        seller = SellerFactory.create(id=79, integration_type=IntegrationType.PERNOD)
        session.commit()
        ...
        json_file = open(
            os.path.join(
                here,
                '../infrastructure/pernod/pernod_response/get_order_status_response.json'))
        response = json.load(json_file)
        active_responses.add(responses.GET, 'https://{}/Orders/menu/12345?access_token=DDDD'
                             .format(os.getenv("PERNOD_API_URL")),
                             json=response, status=200)
        order_sqs_queue = \
            OrderStatusNotificationSQSQueue(url="https://sqs.us-west-2.amazonaws.com/976847220645/order-status-"
                                                "notification-queue")
        ...
        assert result.value.order_id == response['reference']['source']
        assert result.value.status.status.name == response['status']['status'].upper()
