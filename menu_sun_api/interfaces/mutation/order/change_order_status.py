import graphene

from menu_sun_api.domain.model.order.order import OwnerType
from menu_sun_api.interfaces.definition.failure_message import FailureMessage
from menu_sun_api.domain.model.order.order_repository import OrderRepository
from menu_sun_api.infrastructure.connection_factory import Session
from menu_sun_api.command.order.order_status_command import OrderStatusCommand, OrderStatusCommandHandler


class ChangeOrderStatus(graphene.Mutation):
    class Arguments:
        order_id = graphene.String(required=True)
        comments = graphene.String(required=True)
        status = graphene.String(required=True)

    order_id = graphene.String(required=True)
    comments = graphene.String(required=True)
    status = graphene.String(required=True)
    failure_message = graphene.Field(FailureMessage)

    def mutate(parent, info, order_id, comments, status, **args):
        session = Session()
        seller = info.context.get('seller')
        order_repository = OrderRepository()
        command = OrderStatusCommand(seller_id=seller.id, order_id=order_id, comments=comments, status=status,
                                     owner=OwnerType.MENU)
        handler = OrderStatusCommandHandler(
            order_repository=order_repository, session=session)
        rs = handler.execute(command)
        if rs:
            return ChangeOrderStatus(order_id=order_id, failure_message=None)
        else:
            return ChangeOrderStatus(order_id=order_id, failure_message=rs)


class ChangeOrderStatusMutation(graphene.ObjectType):
    change_order_status = ChangeOrderStatus.Field()
