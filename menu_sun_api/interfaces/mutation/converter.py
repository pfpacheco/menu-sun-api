...
from graphene_sqlalchemy.fields import createConnectionField
...
def convert_sqlalchemy_hybrid_method(hybrid_item):
    return String(description=getattr(
        hybrid_item, "__doc__", None), required=False)
...
def convert_sqlalchemy_composite(composite, registry):
    ...
    if not converter:
        try:
            ...
            # handle fields that are not attached to a class yet (don't have a
            # parent)
            raise Exception(
...
def convert_sqlalchemy_column(column, registry=None):
    return convert_sqlalchemy_type(
        getattr(column, "type", None), column, registry)
