...
class OrderPlatformService(AbstractPlatformService):
    def __init__(self, platform_service: AbstractPlatformQueue, order_repository: OrderRepository, session):
        super().__init__('order', platform_service=platform_service, repository=order_repository, session=session)

    def enqueue(self, seller):
        super().bind_logger(integration_type=seller.integration_type.name, entity="order",
        ...
        if not specification:
            self._logger.warn(key='order_platform_service', description="specification_not_implemented",
                              payload=seller)
            return
            ...
            for order in orders:
                try:
                    ...
                    self._logger.info(entity_id=order.order_id,
                                      key='order_platform_service_body_dict_to_json',
                                      description="order_platform_service_body",
                                      payload="dict to json ---> {0}".format(order_dict))
                    body = json.dumps(order_dict)
                    self._logger.info(entity_id=order.order_id,
                                      key='order_platform_service', description="enqueue_order",
                                      payload=order_dict)
                    ...
                    if not message_id:
                        ...
                        self._logger.info(entity_id=order.order_id,
                                          key='order_platform_service', description="enqueued_order",
                                          payload=order_dict)

                except Exception as e:
                    self._logger.error(entity_id=order.order_id,
                                       key='order_platform_service',
                                       description="enqueue_order_error_exception_detail",
                                       payload=e)
